...
def actual_default_model_config():
    """Loads the default model configuration from the actual YAML file using get_model_config(None)."""
    config = get_model_config(None) # config_key=None으로 기본 설정 로드
    if config is None:
        # 실패 메시지 업데이트
        pytest.fail("Failed to load actual default model configuration using get_model_config(None). Check config/model_configurations.yaml and its 'default_model_config_key'.")
    return config
...
def test_analyze_review_successful_real_llm_call(valid_state_input, actual_default_model_config): # mocker 제거
    ...
    기본 모델 설정을 사용합니다.
    """
    # selected_model_config_key에 None을 전달하여 기본 설정 사용
    ...
    # used_model_config_key가 None이 아닌 문자열인지(실제 기본 키가 사용되었는지) 확인
    assert isinstance(result["used_model_config_key"], str) and result["used_model_config_key"], \
        "used_model_config_key should be a non-empty string (the actual default key used)"
    
    print(f"\nReal LLM call result summary for default key '{result['used_model_config_key']}': {result['analysis_output']}") 